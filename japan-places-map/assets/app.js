...
const DATA_URL = `${"/japan-places-map/"}japan_places_map_data.json`;
const categoryGroup = (category) => {
  ...
  if (/Природа|парк|Пещера|Животные|Зоопарк/.test(category))
    return "Природа и животные";
  if (/Архитектура|Руины|Улица|Транспорт|Фото/.test(category))
    return "Архитектура и места";
  if (/Еда|Магазин|Онсэн|Отель/.test(category)) return "Еда, шопинг и отдых";
  return "Развлечения и другое";
};
const durationGroup = (minutes) => minutes <= 60 ? "short" : minutes <= 180 ? "half" : minutes <= 480 ? "day" : "trip";
const carGroup = (value) => ["easy", "possible"].includes(value) ? "transit" : ["helpful", "car_recommended"].includes(value) ? "car-helpful" : "car-needed";
const formatDuration = (minutes) => minutes < 60 ? `${minutes} мин` : minutes < 1440 ? `${Math.round(minutes / 6) / 10} ч` : `${Math.round(minutes / 1440)} дн`;
const modeLabel = {
  train: "поезд",
  subway: "метро",
  bus: "автобус",
...
function MapView({
  ...
  const markers = reactExports.useRef(/* @__PURE__ */ new Map());
  const routeLine = reactExports.useRef(null);
  const [ready, setReady] = reactExports.useState(false);
  reactExports.useEffect(() => {
    let alive = true;
    void __vitePreload(() => import("./leaflet-src.js").then((n) => n.l), true ? [] : void 0).then((L) => {
    ...
    void __vitePreload(() => import("./leaflet-src.js").then((n) => n.l), true ? [] : void 0).then((L) => {
      ...
      places.forEach((place) => {
        const marker = L.circleMarker(
          ...
          {
            renderer,
            radius: selected?.id === place.id ? 9 : 6,
            color: selected?.id === place.id ? "#fff" : "#12231d",
            weight: selected?.id === place.id ? 3 : 1.5,
            fillColor: selected?.id === place.id ? "#e65037" : "#d7ff5f",
            fillOpacity: 1
          }
        ).addTo(map.current).on("click", () => onSelect(place));
        marker.bindTooltip(
          `<strong>${place.name}</strong><br>${place.prefecture.ru} · ${categoryGroup(place.category)}`,
          { direction: "top", offset: [0, -5] }
        );
        markers.current.set(place.id, marker);
      });
      if (places.length && places.length < 80 && !selected && !activeRoute)
        map.current.fitBounds(
          L.latLngBounds(places.map((p) => [p.location.lat, p.location.lng])),
  ...
  reactExports.useEffect(() => {
    ...
    void __vitePreload(() => import("./leaflet-src.js").then((n) => n.l), true ? [] : void 0).then((L) => {
      ...
      const points = activeRoute.placeIds.map((id) => lookup.get(id)).filter(Boolean);
      if (points.length > 1) {
        routeLine.current = L.polyline(
          points.map((p) => [p.location.lat, p.location.lng]),
          { color: "#e65037", weight: 4, opacity: 0.85, dashArray: "8 8" }
        ).addTo(map.current);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ref: el,
      className: "map",
      "aria-label": "Интерактивная карта мест Японии"
    }
  );
}
function Home() {
  ...
  reactExports.useEffect(() => {
    ...
    fetch(DATA_URL, { signal: controller.signal }).then((response) => {
      if (!response.ok) {
  ...
  const route = reactExports.useMemo(
    () => data?.routes.find((r) => r.id === filters.route) ?? null,
    [data, filters.route]
  );
  const routePlaceIds = reactExports.useMemo(() => new Set(route?.placeIds ?? []), [route]);
  const filtered = reactExports.useMemo(
    () => data?.places.filter((p) => {
      const q = filters.query.toLowerCase().trim();
      return (!filters.prefecture || p.prefecture.ru === filters.prefecture) && (!filters.category || categoryGroup(p.category) === filters.category) && (!filters.access || (filters.access.startsWith("score-") ? p.access.score === Number(filters.access.at(-1)) : carGroup(p.access.carNeed) === filters.access)) && (!filters.transport || p.access.modes.includes(filters.transport)) && (!filters.duration || durationGroup(p.visit.minutes) === filters.duration) && (!filters.route || routePlaceIds.has(p.id)) && (!q || `${p.name} ${p.summary} ${p.tags.join(" ")} ${p.area.ru} ${p.area.jp}`.toLowerCase().includes(q));
    }) ?? [],
    [data, filters, routePlaceIds]
  );
  const prefectures = reactExports.useMemo(
    () => [...new Set(data?.places.map((p) => p.prefecture.ru) ?? [])].sort(
  ...
  const categories = reactExports.useMemo(
    () => [
      ...new Set(data?.places.map((p) => categoryGroup(p.category)) ?? [])
    ].sort(),
    [data]
  );
  const routes = reactExports.useMemo(
    () => data?.routes.filter(
      (r) => !filters.prefecture || r.prefectures.includes(filters.prefecture)
    ) ?? [],
    [data, filters.prefecture]
  );
  const activeCount = Object.values(filters).filter(Boolean).length;
  if (loadError) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "loading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Карта не загрузилась" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: loadError })
    ] });
  }
  if (!data) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "loading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "loader" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Загружаем карту…" })
    ] });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "app-shell", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "topbar", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "brand", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "brand-mark", children: "日" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Japan Places" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: "карта необычных мест" })
        ] })
      ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          children: [
            "Фильтры ",
            activeCount ? /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: activeCount }) : null
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stat", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: data.places.length }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "мест" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stat", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: data.routes.length }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "маршрутов" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: `sidebar ${filtersOpen ? "open" : ""}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sidebar-title", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "НАСТРОИТЬ ПОЕЗДКУ" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "Куда едем?" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => setFiltersOpen(false), children: "×" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Filter,
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Filter,
        {
          ...
          options: [
            ["transit", "Легко без машины"],
            ["car-helpful", "Машина поможет"],
            ["car-needed", "Машина нужна"],
            ...[1, 2, 3, 4, 5].map((n) => [`score-${n}`, `Сложность ${n} / 5`])
          ],
          empty: "Любая сложность"
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Filter,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Filter,
        {
          label: "Готовый маршрут",
          value: filters.route,
          onChange: (v) => update("route", v),
          options: routes.map((r) => [
            r.id,
            `${r.name} · ${formatDuration(r.durationMin)}`
          ]),
          empty: "Без маршрута"
        }
      ),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "source-note", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "●" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Данные проверены" }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "map-panel", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        MapView,
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "map-result", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: filtered.length }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          filtered.length === 1 ? "место" : "мест",
          " на карте"
        ] })
      ] }),
      route && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "route-banner", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "МАРШРУТ" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: route.name }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: route.suggestedOrder })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "results", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "results-head", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "ПОДХОДЯЩИЕ МЕСТА" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("h2", { children: [
            filtered.length,
            " находок"
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: "Нажмите, чтобы показать на карте" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "cards", children: [
        filtered.slice(0, 80).map((p) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          PlaceCard,
          {
            place: p,
        ...
        !filtered.length && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: "Ничего не нашлось" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Попробуйте ослабить один из фильтров." })
        ] }),
        filtered.length > 80 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "more", children: "На карте показаны все точки. В списке — первые 80." })
      ] })
    ] }),
    selected && /* @__PURE__ */ jsxRuntimeExports.jsx(PlaceDrawer, { place: selected, onClose: () => setSelected(null) })
  ] });
}
function Filter({
  label,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "filter", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value, onChange: (e) => onChange(e.target.value), children: [
      ...
      options.map(([v, l]) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: v, children: l }, v))
    ] })
  ] });
}
function PlaceCard({
    ...
    "button",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-top", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "category-dot" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: categoryGroup(place.category) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: place.access.score })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: place.name }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: place.summary }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-meta", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
            "◷ ",
            formatDuration(place.visit.minutes)
          ] })
        ] })
      ]
    }
...
function PlaceDrawer({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "drawer", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "drawer-close", onClick: onClose, "aria-label": "Закрыть", children: "×" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "eyebrow", children: [
      categoryGroup(place.category),
      " · доступ ",
      place.access.score,
      "/5"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: place.name }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "jp-name", children: [
      place.area.jp,
      " · ",
      place.prefecture.jp
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "lead", children: place.interest }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "facts", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "ВРЕМЯ" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: formatDuration(place.visit.minutes) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "ОТ СТАНЦИИ" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
          place.location.stationWalkMin,
          " мин"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "СРЕДА" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: environmentLabel[place.visit.indoorOutdoor] ?? place.visit.indoorOutdoor })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Как добраться" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: place.access.note }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: place.access.modes.map((m) => modeLabel[m] ?? m).join(" · ") })
    ] }),
    place.visit.hoursNote && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Часы и сезон" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: place.visit.hoursNote }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: place.visit.seasonality })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "drawer-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: place.links.googleMaps, target: "_blank", rel: "noreferrer", children: "Открыть в Google Maps ↗" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "a",
        {
          className: "secondary",
          href: place.links.officialOrSource,
          target: "_blank",
          rel: "noreferrer",
...
clientExports.createRoot(root).render(
  /* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.StrictMode, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(Home, {}) })
);
